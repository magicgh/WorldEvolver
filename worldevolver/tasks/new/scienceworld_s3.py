...
class ScienceworldS3(CLSWMTaskMixin, EvalScienceworld):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not wm_name:
            ...
            self.wm = build_wm(
                wm_name, llm=agent_llm, env_name="scienceworld", **self.wm_config,
            )
    ...
    def _run_single_trial(self, index, trial, task_name, var, modified_goal):
        ...
        logger.info(
            "Stage3 SW | Example {} | Trial {} | wm={} | Init obs: {}".format(
                index, trial, self.wm_name, init_obs,
            )
        )
        ...
        for i in range(self.max_num_steps):
            ...
            if self.uses_itp_foresight():
                ...
                self.maybe_inject_foresight(
                    modified_goal, "scienceworld", trajectory, i, state=last_wm_state,
                )
    ...
    def evaluate(self):
        ...
        for index, (_label_key, v) in enumerate(list(labels.items())[: self.num_exams]):
            ...
            for trial in range(self.num_trials):
                score, done, gr_acc, record, num_steps = self._run_single_trial(
                    index, trial, task_name, var, modified_goal,
                )
                ...
                logger.finish(
                    "Stage3 SW | Example {} | Trial {} | wm={} | done={} pr={:.3f} steps={}".format(
                        index, trial, self.wm_name, done, score, num_steps + 1,
                    )
                )
