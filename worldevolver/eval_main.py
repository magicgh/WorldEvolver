...
def main():
    ...
    _override_ctx = os.environ.get("WORLDEVOLVER_LLM_CONTEXT_LENGTH")
    if _override_ctx:
        llm_config["context_length"] = int(_override_ctx)
    _override_base_url = os.environ.get("WORLDEVOLVER_LLM_BASE_URL")
    if _override_base_url:
        llm_config["base_url"] = _override_base_url
    _override_max_tokens = os.environ.get("WORLDEVOLVER_LLM_MAX_TOKENS")
    if _override_max_tokens:
        llm_config["max_tokens"] = int(_override_max_tokens)
    ...
    run_logger = SummaryLogger(baseline_dir=baseline_path, log_path=log_dir)
    ...
    for task_name in task_names:
        ...
        if task_name in log_history:
            ...
            run_logger.log_run_result(task_name, log_history[task_name]["success_rate"], log_history[task_name]["progress_rate"], log_history[task_name]["grounding_acc"],
        ...
        run_logger.log_run_result(task_name, success_rate, progress_rate, grounding_acc, hard_sr, easy_sr, hard_pr, easy_pr)
