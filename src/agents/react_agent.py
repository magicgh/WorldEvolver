...
_MPO_THOUGHT_ACTION_RE = re.compile(
    r"(?:Thought|Think):\s*(.+?)(?:\n|\.\s+)\s*Action:\s*(.+?)(?:\n|$)",
    re.S,
)
...
class ReactAgent(
    ...
    def __init__(self,
        ...
        self.split = getattr(self.llm_model, "xml_split", None) or {
            "example": [""],
            "text": [""],
            "rule": [""],
            "system_msg": [""],
    ...
    def extract_think(self, response):
        ...
        for prefix in ("Thought: ", "Think: "):
            if response.startswith(prefix):
                return response.split(prefix, 1)[-1].split("\n")[0]
        return None
    ...
    def make_prompt(self,
        ...
        if need_goal:
            ...
            query += self.split["goal"][0] + "Your task is to: " + self.goal + "\n" + \
                     self.split["goal"][-1]
        ...
        def _build_prompt(hist):
            ...
            while i < n:
                tag, content = hist[i]
                if (tag == self.REASONING_TAG
                        and i + 1 < n
                        and hist[i + 1][0] == "Action"):
                    next_tag, next_content = hist[i + 1]


                    lines.append(
                        f"{tag}: {content.rstrip('. ')}. {next_tag}: {next_content}"
                    )
                    i += 2
                else:
        ...
        budget = self.max_context_length - self.llm_model.max_tokens
        input_prompt = _build_prompt(history)
        encoding = getattr(self.llm_model, "_encoding", None)
        if encoding is None:
            messages = [
            ...
            while num_of_tokens > budget and history:
                history = history[1:]
                input_prompt = _build_prompt(history)
                messages = [
                    {"role": "system", "content": system_message},
        ...
        sys_tokens = len(encoding.encode(system_message)) + 3


        user_tokens = len(encoding.encode(input_prompt)) + 3
        num_of_tokens = sys_tokens + user_tokens
        ...
        while num_of_tokens > budget and history:
            ...
            dropped_text = dropped[0] + ": " + dropped[1] + "\n"
            num_of_tokens -= len(encoding.encode(dropped_text))
    ...
    def run(self, init_prompt_dict=None):
        ...
        while not flag:
            ...
            mpo_match = _MPO_THOUGHT_ACTION_RE.search(response)
            if mpo_match:
                thought_text = mpo_match.group(1).strip()
                action_text  = mpo_match.group(2).strip()
                flag = True
                self.think_count = 0
                self.force_action = False


                self.memory.append((self.REASONING_TAG, thought_text))
            ...
            if response.startswith("Action"):
            ...
            elif response.startswith(("Thought:", "Think:")):
                ...
                print("Step {:02} - {}: {}".format(self.steps, self.REASONING_TAG, response))
                self.memory.append(
                    (self.REASONING_TAG, response)
                )
