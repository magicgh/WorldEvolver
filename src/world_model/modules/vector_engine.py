...
_VECTOR_INDEX_DEVICE = "cpu"


def configure_vector_index_device(device: str = "cpu") -> None:
    global _VECTOR_INDEX_DEVICE
    spec = str(device or "cpu").strip().lower()
    if spec != "cpu" and re.match(r"^cuda:\d+$", spec) is None:
        raise ValueError("vector_index_device must be 'cpu' or 'cuda:<id>'")
    _VECTOR_INDEX_DEVICE = spec


def _parse_vector_index_gpu_id() -> Optional[int]:

    spec = _VECTOR_INDEX_DEVICE
