...
class LLMFactorizer:
    ...
    def checkpoint_state(self) -> Dict[str, Any]:
        return {
            "cache": {
                ...
                for observation, (triples, ok) in self._cache.items()
            },
    ...
    def load_checkpoint_state(self, state: Dict[str, Any]) -> None:
        if not isinstance(state, dict):
            raise ValueError("factorizer checkpoint must be an object")
        cache: Dict[str, Tuple[List[List[str]], bool]] = {}
        for observation, value in (state.get("cache") or {}).items():
            if not isinstance(value, dict):
                raise ValueError("invalid factorizer cache entry")
            triples = value.get("triples") or []
            if not isinstance(triples, list):
                raise ValueError("invalid factorizer triples")
            normalized = []
            for triple in triples:
                if not isinstance(triple, (list, tuple)) or len(triple) < 3:
                    raise ValueError("invalid factorizer triple")
                normalized.append(list(map(str, triple[:3])))
            cache[str(observation)] = (
                normalized,
                bool(value.get("ok")),
            )
        self._cache = cache
