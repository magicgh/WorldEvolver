...
_EMBED_CONFIG: dict[str, Any] = {
    "base_url": None,
    "model": _QWEN3_EMBED_8B,
    "api_key": None,
...
def configure_embedding_client(
    *,
    ...
    """Configure the process-local embedding client from explicit settings."""
    if not str(base_url or "").strip():
        raise ValueError("embed_base_url must be provided explicitly")
    if not str(model or "").strip():
        raise ValueError("embed_model must be a non-empty string")
    global _EMBEDDER
    with _EMBEDDER_LOCK:
        _EMBED_CONFIG.update(
            {
...
def _embedding_client():
    ...
    with _EMBEDDER_LOCK:
        if _EMBEDDER is not None:
            return _EMBEDDER
        base_url = _EMBED_CONFIG["base_url"]
        if not base_url:
            raise RuntimeError(
                "Embedding endpoint is not configured; pass embed_base_url "
                "in wm_config or configure the analysis CLI explicitly."
            )
        _EMBEDDER = _OpenAIEmbeddingClient(
            base_url=base_url,
            model=_EMBED_CONFIG["model"],
            api_key=_EMBED_CONFIG["api_key"],
            timeout=_EMBED_CONFIG["timeout"],
        )
        return _EMBEDDER
