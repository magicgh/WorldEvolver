...
_SECRET_CONFIG_KEYS = {"api_key", "authorization", "token"}
...
def _redact_secrets(value):
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            name = str(key).lower()
            is_secret = (
                name in _SECRET_CONFIG_KEYS
                or name.endswith("_api_key")
                or name.endswith("_token")
            )
            redacted[key] = "<redacted>" if is_secret else _redact_secrets(item)
        return redacted
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run WorldEvolver evaluations")
    ...
    parser.add_argument("--log_path", required=False, default='', help="specify where to store the results")
