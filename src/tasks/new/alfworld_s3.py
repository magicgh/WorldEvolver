...
class AlfWorldS3(CLSWMTaskMixin, Evalalfworld):
        ...
        **kwargs,
        ...
        self.resume_config_fingerprint = experiment_config_fingerprint(
            ...
            kwargs.get("env_config", {}),
            {
                ...
                "num_exams": kwargs.get("num_exams"),
                "seed": seed,
    ...
    def _run_single_trial(self, index, trial, ob, examples):
        ...
        for i in range(self.max_num_steps):
            ...
            if done:
                ...
                self._dump_trace(
                    index, trial, trajectory, success=True, progress=reward
                )
    ...
    def evaluate(self):
        ...
        game_files = evaluation_order(
            game_files[: self.num_exams],
            shuffle=self.shuffle_evaluation_order,
            seed=self.seed,
        )
        self.evaluation_order = [str(gamefile) for gamefile in game_files]
        ...
        for idx, gamefile in enumerate(game_files):
            if idx < start_index:
                continue
            ran_task = True
            self.reset_stage3_world_model_memory(
            ...
            for trial in range(self.num_trials):
                ...
                try:
                    ...
                    if task_difficulty is None:
                        task_difficulty = self.env.difficulty
                    ...
                    for k, v in prefixes.items():
                        if trial_name.startswith(k):
                            ...
                            score, is_done, gr_acc, record, steps = (
            ...
            if not task_trial_results:
                ...
                raise RuntimeError(f"Stage 3 ALF: no prefix matched {trial_name}")
            difficulties.append(task_difficulty)
    ...
    def from_config(cls, run_config, llm_config, agent_config, env_config, llm=None):
        ...
        init_prompt_path = agent_config.get(
            "init_prompt_path",
            "prompts/alfworld_in_context_learning.json",
        )
