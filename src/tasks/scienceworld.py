...
class EvalScienceworld(BaseTask):
    def __init__(self,
        ...
        self.num_exams = int(run_config.get("num_exam", 90))
        self.context_length = llm_config.get("context_length")

        self.baseline_dir = baseline_dir

        self.run_logger = TaskLogger(task_name="scienceworld", log_path=log_path, max_num_steps=self.max_num_steps, baseline_dir=self.baseline_dir)
    ...
    def evaluate_env(self, index, task_name, var, modified_goal):
        ...
        for i in range(self.max_num_steps):
            ...
            if isDone:
                ...
                self.run_logger.log_example(index, True, 1.0, grounding_acc_count / (i + 1), score_change_record, env_details, trajectory)
        ...
        self.run_logger.log_example(index, isDone, progress_rate, grounding_acc_count / (i + 1), score_change_record, env_details, trajectory, example_prompt)
    ...
    def evaluate(self):
        ...
        for index, (k, v) in enumerate(list(labels.items())[: self.num_exams]):
        ...
        self.run_logger.log_summary(sr, pr, gr, score_state_records, hard_sr, hard_pr, easy_sr, easy_pr)
