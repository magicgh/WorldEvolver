    ...
    def log_summary(self):
        ...
        for task_name in CATEGORIES:
            for model_name in all_results[task_name]:
                ...
                result_df = pd.concat([result_df, pd.DataFrame([{"model_name": model_name, "task_name": task_name, "success_rate": 100 * all_results[task_name][model_name]["success_rate"], "baseline": dash}])], ignore_index=True)
        ...
        for dimension in DIMENSION_CATEGORIES:
            ...
            for model_name in all_results[list(CATEGORIES)[0]]:
                ...
                dash = False if model_name == "Current Run" else True
                dimension_df = pd.concat([dimension_df, pd.DataFrame([{"model_name": model_name, "dimension": dimension, "score": score, "baseline": dash}])], ignore_index=True)


        if len(dimension_df) > 0 and "baseline" in dimension_df.columns:
...
class TaskLogger:
    def __init__(self, task_name, log_path, max_num_steps=30, baseline_dir= "data/baseline_results", resume=False):
    ...
    def extract_variables(self, line):
        ...
        if match:
            ...
            score_state = ast.literal_eval(score_state_str)
            if not isinstance(score_state, list):
                raise ValueError("score_state must be a list")
    ...
    def log_example(self, id, is_done, reward, grounding_accuracy, score_change_record, env_details, trajectory, example_prompt=None):
        self.save_sample_data_to_file_detailed(id, is_done, reward, grounding_accuracy, score_change_record, env_details, trajectory, example_prompt)
        self.save_sample_data_to_file_overview(id, is_done, reward, grounding_accuracy, score_change_record, env_details, trajectory)

        self.log_example_data(id, is_done, reward, grounding_accuracy, score_change_record, env_details, trajectory)
        self.update()
    ...
    def log_summary(self, success_rate, reward_score, grounding_acc, score_steps, hard_sr=None, hard_rs=None, easy_sr=None, easy_rs=None):
        ...
        for step, score in enumerate(reward_score_list):
            df = pd.concat([df, pd.DataFrame([{"models": "Current Run", "steps": step, "score": score, "baseline": False}])], ignore_index=True)


        for model in list(self.baseline_metrics.keys()):
            for step, score in enumerate(self.baseline_reward_wrt_step[model]):
                df = pd.concat([df, pd.DataFrame([{"models": model, "steps": step, "score": score, "baseline": True}])], ignore_index=True)
